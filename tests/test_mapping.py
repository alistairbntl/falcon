...
def test_reference_element_mesh_r1(basic_mesh_2):
    ...
    reference_map = mp.ReferenceElementMap(element)
    assert reference_map.apply_affine_map(0,0).vals == (0,0)
    assert reference_map.apply_affine_map(1,0).vals == (0,1)
    assert reference_map.apply_inverse_affine_map(0,0) == (0,0)
    assert reference_map.apply_inverse_affine_map(0,1) == (1,0)
    assert abs(reference_map.get_jacobian_det()-0.64644661) < 1.0e-6
