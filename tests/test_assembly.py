...
@pytest.mark.bdm3_fix
def test_mixed_assembly_bdm3(basic_mesh_s1_bdm1_partial_r1):
    basis, mesh, dof_handler = basic_mesh_s1_bdm1_partial_r1
    ...
    for eN in range(num_mesh_elements):
        ...
        for quad_pt in quadrature.get_element_quad_pts():
            ...
            for i in range(test_space[0].get_num_dof()):
                ...
                ele_quad_pt = quadrature.get_quad_on_element(piola_map,
                                                             quad_pt)
                vals = true_solution.get_f_eval(ele_quad_pt)
                int_val = (la.Operators.dot_product(vals,
                                                    val_dic_test['Jt_vals'])
                           * (1./val_dic_test['|Jt|'])
                           * ele_quad_pt.get_quad_weight())
...
def test_mixed_bdm2_assembly_4(basic_mesh_s1_bdm2_partial_r1):
    ...
    p_dx = lambda x,y : -x*y + y**2
    p_dy = lambda x,y : -x -x**2 + 0.5*y**2
...
def test_mixed_bdm3_assembly_1(basic_mesh_s1_bdm3_p2):
    basis, mesh, dof_handler = basic_mesh_s1_bdm3_p2
