...
def test_mixed_assembly_1(basic_mesh_bdm1_p0):
    ...
    assert abs(global_rhs._rhs_vec[0] + 0.43769752) < 1.0E-8
    assert abs(global_rhs._rhs_vec[1] - 0.64455656) < 1.0E-8
    assert abs(global_rhs._rhs_vec[2] - 1.56919528) < 1.0E-8    
    assert abs(global_rhs._rhs_vec[5] - 0.52936418) < 1.0E-8
...
def test_mixed_assembly_2(basic_mesh_s1_bdm1_p0):
    ...
    assert np.array_equal(dof_handler._l2g_map[1], [4.,5.,3.,12.,13.,11.,17])
    assert np.array_equal(dof_handler._l2g_map[2], [6.,5.,1.,14.,13.,9.,18])
    ...
    error_calculator = ec.L2Error(mesh,dof_handler,basis[0],solution_vec)

    # a few tests to check calculation of correction solution from solution_vec
    element=mesh.get_element(0)
    reference_map=mp.ReferenceElementMap(element)
    piola_map=mp.PiolaMap(element)
    physical_pt=reference_map.apply_inverse_affine_map(1.,0.5-math.sqrt(3)/6.)
    quad_pt=mt.QuadraturePoint(physical_pt[0],physical_pt[1])    
    val = error_calculator.get_solution_approx(element,reference_map,piola_map,quad_pt)[0]
    assert global_rhs.get_val(0) == val
    physical_pt=reference_map.apply_inverse_affine_map(1.,0.5-math.sqrt(3)/6.)
    quad_pt=mt.QuadraturePoint(physical_pt[0],physical_pt[1],0.)
    val = error_calculator.get_solution_approx(element,reference_map,piola_map,quad_pt)[0]
    assert abs(global_rhs.get_val(8) - val) <= 1.0e-8

    element=mesh.get_element(2)
    reference_map=mp.ReferenceElementMap(element)
    piola_map=mp.PiolaMap(element)
    physical_pt=reference_map.apply_inverse_affine_map(0.5-math.sqrt(3)/6.,1.)
    quad_pt=mt.QuadraturePoint(physical_pt[0],physical_pt[1])
    val = error_calculator.get_solution_approx(element,reference_map,piola_map,quad_pt)[1]
    assert abs(global_rhs.get_val(6)-val) <= 1.0e-8
    physical_pt=reference_map.apply_inverse_affine_map(0.5 + math.sqrt(3)/6.,1.)
    quad_pt=mt.QuadraturePoint(physical_pt[0],physical_pt[1])
    val = error_calculator.get_solution_approx(element,reference_map,piola_map,quad_pt)[1]
    assert abs(global_rhs.get_val(14)-val) <= 1.0e-8    
...
def test_mixed_assembly_3(basic_mesh_s1_bdm1_p0_tmp):
    ...
    assert abs(global_rhs._rhs_vec[0] - 0.09858439) < 1.0E-8
    assert abs(global_rhs._rhs_vec[1] - 0.01525105) < 1.0E-8
    assert abs(global_rhs._rhs_vec[5] - 0.02641560) < 1.0E-8
